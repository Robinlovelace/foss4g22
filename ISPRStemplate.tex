...
Origin-Destination datasets inform how people travel betewen areas. This is usually represented by connecting two centroids of such areas, and assign that information to a route along the road network. For active travel, this representation is often too generalistic, once that the origins and destinations locations diffusion can bring different representations of a reality, and the route possibilities to travel are broader. This paper uses 'jittering' aproach to a dataset of bike trips in Lisbon, Portugal, and compares the results of a diffuser network with counting cycling data, in order to analize if and which jittering parameters better fits the data, and which routing options are a better estimate of real cycling trips for the case study. The results suggest that a jittered OD dataset with a low level of traffic stress (quieter) better fits the data, establishing a better approach for this mode of transport than the traditional modelling tecnhiques. This results are useful to validate the method and add value to projects with OD cycling datasets based in Ireland, Norway, Portugal, New Zealand and beyond.
}
