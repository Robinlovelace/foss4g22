...
```{r jitteredoverview, echo=FALSE, fig.cap="\\label{poltlisbon}Trips represented with desire lines from centroids and with jittering (no disagregation and with disagregation), for Lisbon (Portugal)", fig.show='hold', fig.ncol=2, out.width="100%"}
...
routes_unjittered_lts4 = readRDS("routes_unjittered_google.Rds")
...
counters_sf_joined = st_join(counters_sf_joined,
                             ...
                             join = sf::st_nearest_feature)
counters_sf_joined = st_join(counters_sf_joined,
                             rnet_unjittered_google %>% rename(Bikes_unjittered_google = Bike),
                             join = sf::st_nearest_feature)
...
Figure 2 shows and example of route network from unjittered OD pairs, and jittered OD pairs with disagregation level of 500 trips, for routing option "quietest", and the counters.


```{r echo=FALSE, message=FALSE, warning=FALSE, fig.ncol=2, out.width="100%"}
library(tmap)
library(biclar)
tmap_mode("view")

# tm_shape(od_lisbon_jittered_500) +
#   tmap::tm_lines(
#     id = NULL,
#     lwd = "Bike",
#     scale = 15,
